...
const generateWeeklyNewsletter = async () => {
  try {
    ...
    const selectedArticles = await getSelectedArticles(
    ...
    const aiNewsletter = await newsletterGenerationByMistral(selectedArticles);
    ...
    // Safety check
    if (!aiNewsletter || !aiNewsletter.articles) {
      throw new Error("Invalid newsletter response from Mistral");
    }

    // to avoid ai from making mistakes
    if (aiNewsletter.articles.length !== 5) {
      throw new Error("Expected exactly 5 articles");
    }

    // to prevent ai from hallucinating article ids
    for (const article of aiNewsletter.articles) {
      if (
        !selectedArticles.some((a) => a._id.toString() === article.articleId)
      ) {
        throw new Error("Unknown articleId returned by AI");
      }
    }

    // Step 6: Merge AI response with DB articles
    const mergedArticles = aiNewsletter.articles.map((item) => {
    ...
    const newsletterDocument = {
      title: `AI Weekly Digest | ${formattedWeekStart} - ${formattedWeekEnd}`,

      weeklySummary: aiNewsletter.weeklySummary,

      weekStart,

      weekEnd,

      selectedArticles: mergedArticles,
    };

    // Step 9: Save newsletter
    const savedNewsletter = await newsletterModel.create(newsletterDocument);
...
const deliverLatestNewsletter = async () => {
  const newsletter = await newsletterModel.findOne().sort({ weekEnd: -1 });
...
export { generateWeeklyNewsletter, deliverLatestNewsletter };
